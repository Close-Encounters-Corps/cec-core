...
type LoginDiscordParams struct {
	...
	*/
	State *string
	/*First phase: URL to redirect on a success of the second phase
	  In: query
	*/
	SuccessURL *string
}
...
func (o *LoginDiscordParams) BindRequest(r *http.Request, route *middleware.MatchedRoute) error {
	...
	qSuccessURL, qhkSuccessURL, _ := qs.GetOK("success_url")
	if err := o.bindSuccessURL(qSuccessURL, qhkSuccessURL, route.Formats); err != nil {
		res = append(res, err)
	}
	if len(res) > 0 {
...
// bindSuccessURL binds and validates parameter SuccessURL from query.
func (o *LoginDiscordParams) bindSuccessURL(rawData []string, hasKey bool, formats strfmt.Registry) error {
	var raw string
	if len(rawData) > 0 {
		raw = rawData[len(rawData)-1]
	}
